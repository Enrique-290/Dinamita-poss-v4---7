...
const UI = {
  ...
  init(){
    ...
    Dashboard.render(); Inventario.renderTabla(); Historial.renderTabla(); Reportes.renderAll?.(); Bodega.init(); Clientes.renderTabla();
  },
  show(id){
    UI.current = id;
    ...
    if(id === 'clientes' && window.Clientes && Clientes.renderTabla) Clientes.renderTabla();
  }
};
...
const Bodega = {
  ...
  sumarRapido(){
    ...
    const p = (state.products || []).find(x => x.sku === this.selectedSku);
    if(!p){ alert('Producto no encontrado'); return; }
    state.bodegaStock = state.bodegaStock || {};
    const cur = state.bodegaStock[p.sku] || { qty:0, costoProm:(p.costo || 0) };
    cur.qty += qty; state.bodegaStock[p.sku] = cur;
    state.movimientosBodega = state.movimientosBodega || [];
    state.movimientosBodega.unshift({ folio:'ER' + Date.now().toString().slice(-6), fecha:new Date().toISOString(), tipo:'entrada_rapida', sku:p.sku, nombre:p.nombre, qty, costo:cur.costoProm || 0, notas:'suma rápida' });
    DB.save(state); this.renderStock(); this.renderMovs(); Dashboard.render();
    alert('Stock en bodega actualizado.');
    const s = document.getElementById('bodQuickSearch'); if(s) s.value = '';
    const q = document.getElementById('bodQuickQty'); if(q) q.value = 1;
    this.selectedSku = '';
  }
};
